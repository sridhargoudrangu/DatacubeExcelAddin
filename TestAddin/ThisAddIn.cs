...
namespace TestAddin
{
	public sealed partial class ThisAddIn : Microsoft.Office.Tools.AddInBase
	{
		...
		private void Load(object sender, EventArgs e)
		{
			...
			// Setup event handlers -- used for workbook synchronization
		...
		private void SheetChangedEvent(object Sh, Range Target)
		{
			if(Sh is Worksheet ws && ws.IsSynchronizedWorksheet())
			{
				...
				{
					// user is deleting a single cell
					// easy case
		...
		private void WorkbookOpenedEvent(Workbook wb)
		{
			...
			if(Authentication.LoggedIn)
				foreach(Worksheet ws in wb.Sheets)
					if(ws.IsSynchronizedWorksheet(out string host, out int dsid, out int handlerid, out string handlername, out string dsname, out DateTime timestamp, out string linkedwsname))
