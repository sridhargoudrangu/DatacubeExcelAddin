...
namespace TestAddin.ACTIONS
{
	...
	*/
	static class SynchronizedWorksheet
	{
		...
		/// <summary>Check if the worksheet is synchronized using the presence of the cvlt_sync flag</summary>
		public static bool IsSynchronizedWorksheet(this Worksheet ws)
		{
			try
			{
				...
				// .Item[i] NEEDS AN INDEX i >= 1
				// LOOP THROUGH IT TO FIND cvlt_sync
				return ws.CustomProperties.Item["cvlt_sync"].Value;
				// You cannot assume that the cvlt_sync key will be in position 1:
				// Another addin might get to the worksheet first
			}
			catch
			{
		...
		public static bool GetSyncValues(this Worksheet ws, out string host, out int dsid, out int handlerid, out string handlername, out string dsname, out DateTime timestamp, out string cid_ws_name)
		{
			if(ws.IsSynchronizedWorksheet())
			{
				// .Item[x] requires x to be an integer
				// must loop through all values to find these keys.
		...
		public static string GetContentId(this Worksheet ws, int row)
			=> ws.GetSyncValues(out string host, out int dsid, out int handlerid, out string handlername, out string dsname, out DateTime timestamp, out string tgt) ? ((Worksheet)Useful.Worksheets[tgt]).Cell(row, 1).Value2 as string : null;
